...
// This is our new core renderer.
const WhiteboardStep = ({ step }) => {
  const { type, content, src, alt, animation, items } = step;

  const renderContent = () => {
    switch (type) {
      // TEXT uses TypeAnimation with the handwriting font (applied via CSS)
      case 'heading':
        return <TypeAnimation sequence={[content]} speed={40} wrapper="h1" cursor={false} />;
      case 'sub-heading':
        return <TypeAnimation sequence={[content]} speed={50} wrapper="h2" cursor={false} />;
      case 'text':
        return <TypeAnimation sequence={[content]} speed={60} wrapper="p" cursor={false} />;
      case 'list':
        return (
          <ul className="checklist">
            {items.map((item, i) => (
              <li key={i}>
                <TypeAnimation sequence={[item]} speed={60} wrapper="span" cursor={false} />
              </li>
            ))}
          </ul>
        );
      
      // SVG uses a special container for the drawing animation
      case 'svg':
        return <div className="hand-drawn-svg" dangerouslySetInnerHTML={{ __html: content }} />;

      // FORMULAS & IMAGES pop in cleanly
      case 'formula':
  ...
  // The 'pop-in' animation class is now used for formulas and images for a clean effect
  return (
    ...
    <div className="whiteboard-overlay" onClick={onClose}>
      <div className="whiteboard-container" onClick={(e) => e.stopPropagation()}>
        <header className="whiteboard-header">
          {/* Header font should not be handwritten for clarity */}
          ...
          <button className="whiteboard-close-btn" onClick={onClose} title="Close Explanation">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="m12 13.4l-4.9 4.9q-.275.275-.7.275t-.7-.275q-.275-.275-.275-.7t.275-.7L10.6 12L5.7 7.1q-.275-.275-.275-.7t.275-.7q.275-.275.7-.275t.7.275L12 10.6l4.9-4.9q.275-.275.7-.275t.7.275q.275.275.275.7t-.275.7L13.4 12l4.9 4.9q.275.275.275.7t-.275.7q-.275.275-.7.275t-.7-.275z"/></svg>
          </button>
        </header>
